  ...
  return (
    <SizeSpace className=" px-6 py-4" direction="vertical">
      ...
      <WrapperSection title="个人技能" show={!!data?.skillList?.length}>
        { data?.skillList?.map((skill, index) => <SKillItem {...skill} key={skill.skill_name} onChangeAiText={(newKey, aiText) => changeAiText('skillList', { ...skill, [newKey]: aiText }, index)} />) }
      </WrapperSection>
      ...
      <WrapperSection title="个人作品" show={!!data?.workList?.length}>
        { data?.workList?.map((work, index) => <WorkItem {...work} key={work.work_name} onChangeAiText={(newKey, aiText) => changeAiText('workList', { ...work, [newKey]: aiText }, index)} />) }
      </WrapperSection>
